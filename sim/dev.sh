...
# Bootstrap the flash backing image on a fresh clone -- it is gitignored, so a new
# checkout won't have it and Renode would fail to load. Only auto-create the default;
# a custom FLASH_IMAGE (as the tests set) must be generated by its owner.
if [[ ! -f "$FLASH_IMAGE" ]]; then
    if [[ "$FLASH_IMAGE" == "$(realpath -m sim/data/spi_PY25Q16.bin)" ]]; then
        step "creating flash image"
        if python3 sim/make_flash_image.py > "$LOG_DIR/mkimage.log" 2>&1; then
            ok "sim/data/spi_PY25Q16.bin"
        else
            printf 'FAILED\n\n'; tail -20 "$LOG_DIR/mkimage.log"; exit 1
        fi
    else
        echo "flash image not found: $FLASH_IMAGE"
        echo "  create it with: python3 sim/make_flash_image.py --out-dir <dir>"
        exit 1
    fi
fi
